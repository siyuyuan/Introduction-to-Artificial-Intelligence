...
class ExactInference(object):
    ...
    def observe(self, agentX, agentY, observedDist):
        ...
        for row in range(self.belief.getNumRows()):
            for column in range(self.belief.getNumCols()):
                ...
                average = math.sqrt((agentX - X) * (agentX - X) + (agentY - Y) * (agentY - Y) )
    ...
    def elapseTime(self):
        ...
        for row in range(self.belief.getNumRows()):
            for column in range(self.belief.getNumCols()):
                X[(row, column)] = self.belief.getProb(row, column)
                self.belief.setProb(row, column, 0)
        for key in self.transProb.keys():
            (old, new) = key
            self.belief.addProb(new[0], new[1], X[old] * self.transProb[key])
        self.belief.normalize()
...
class ParticleFilter(object):
    ...
    def observe(self, agentX, agentY, observedDist):
        ...
        for par in self.particles:
            P = self.particles[par]
            (row, column) = par
            X, Y = util.colToX(column), util.rowToY(row)
            average = math.sqrt((agentX - X) * (agentX - X) + (agentY - Y) * (agentY - Y))
            cond = util.pdf(average, Const.SONAR_STD, observedDist)
            updateDict[par] = P * cond
        self.particles = collections.defaultdict(int)
        for i in range(self.NUM_PARTICLES):
            new_par = util.weightedRandomChoice(updateDict)
            self.particles[new_par] = self.particles[new_par] + 1
        # raise Exception("Not implemented yet")
        # END_YOUR_CODE
    ...
    def elapseTime(self):
        ...
        for particle in self.particles:
            for i in range(self.particles[particle]):
                new_par = util.weightedRandomChoice(self.transProbDict[particle])
